...
const supabaseContacts = {
    ...
    async getAll() {
        const {data} = await supabaseContacts.supabase.from("test").select()
        console.log({data})
        return data;
    },
    ...
    async create(values: ContactMutation) {
        const data = await supabaseContacts.supabase.from("test").upsert(values)
        console.log("create", {values}, {data})
        return data;
    },
...
const contactsService = false ? fakeContacts : supabaseContacts;
...
export async function updateContact(id: string, updates: ContactMutation) {
    ...
    await contactsService.set(id, {...contact, ...updates});
    return contact;
}

export async function deleteContact(id: string) {
    contactsService.destroy(id);
}
...
[
    ...
    fakeContacts.create({
        ...contact,
        slug: `${contact.first.toLowerCase()}-${contact.last.toLocaleLowerCase()}`,
    });
});